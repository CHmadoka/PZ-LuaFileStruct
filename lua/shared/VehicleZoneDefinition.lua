...
车辆类型应为您在 WorldEd 的区域中定义的类型（VehicleZoneDistribution.trailerpark 将用于拖车公园区域类型）。如果在区域中未定义类型，则使用停车位（parkingstall）。

在添加车辆时，您需要定义其在车辆模板中定义的皮肤索引（-1 表示在皮肤列表中随机选择一个皮肤）。spawnChance 用于定义生成此车辆或其他车辆的几率（区域的总几率应始终为 100）。

您可以使用一系列变量来配置您的生成逻辑：

chanceToPartDamage：部件损坏的几率，此数字将添加到部件的库存物品损坏生成几率中（因此在同一区域内，旧轮胎比好的轮胎更有可能损坏）。默认值为 0。
baseVehicleQuality：定义部件的基础质量，如果部件应生成为损坏状态，这将定义其最大状态（因此 0.7 表示如果部件生成为损坏状态，其最大状态为 70%）。默认值为 1.0。
chanceToSpawnSpecial：用于定义在某区域生成特殊车辆（在所有类型中随机挑选 specialCar = true 的车辆）的几率。默认值为 5。
chanceToSpawnBurnt：用于定义在某区域生成烧毁车辆（在 normalburnt 列表中随机挑选 80%，在 specialburnt 列表中随机挑选 20%）的几率，如在废车场中。默认值为 0。
chanceToSpawnNormal：用于定义在某区域生成普通车辆的几率（将在停车位区域中挑选）。用于确保特殊停车场不仅有特殊车辆（例如，Spiffo 停车场会有很多普通车辆，有时会有 Spiffo 面包车）。默认值为 80%。
spawnRate：在某区域添加车辆的基础几率，默认值为 16%。
chanceOfOverCar：在生成的车辆上生成另一辆车的几率（用于拖车公园）。默认值为 0。
randomAngle：车辆是按网格对齐还是随机角度。默认值为 false。
chanceToSpawnKey：定义为该车辆生成钥匙的几率（可能在地上、直接在车内、在附近的僵尸或容器中）。默认值为 70%。
specialCar：定义车辆是否为特殊车辆（警车、消防车等），用于在触发 chanceToSpawnSpecial 时获取特殊车辆列表。特殊车辆的对应钥匙将不会有颜色。仍然可以作为普通区域使用。
 ]]
